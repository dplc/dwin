...
/*******************************************************************************
        ...
        author:         Kris
                        ...
                        methods added to Arguments.Arguemnt:
                            final Argument help ()
                            final Argument aliased (char[][] name)
        
        Example:
        ---
        args.aliased(["abc", "ABC"]);
        ---
                            ...
                            if (s == "--")
                               {
                                   ...
                                   version(dashdash)
                                    {stack.clear;stack.push(get(null));} 
                                    continue;
                                }
                            else
        ...
        ***********************************************************************/
        ...
        {       
                ...
                ***************************************************************/
                ...
                //added by yidabu   January 26, 2011
                final Argument aliased (char[][] name)
                {
                        foreach(n; name)
                            this.outer.aliases[n] = this;
                        return this;
                }                      
...
/** 
    parse commands for use with tango.text.Arguments
    Date: June 22, 2009
*/
void parseCommands(char[] cmd, ref char[][] cmdArgs)
{
    cmd = trim(cmd);
        ...
        foreach(index, c; cmd)
        {
            if(index > 0 && c == '"')
            {
                if(index == 1)
                    ...
                    cmdArgs ~= cmd[1..index];
                cmd = cmd[index+1..$];
                return parseCommands(cmd, cmdArgs);
            }
        }
    }
...
unittest
{
    ...
    cmd = `--host mydomain.com --port 25 --local "D:\my path"`;
    parseCommands(cmd, cmdArgs);
    assert(cmdArgs.length == 6);
    assert(cmdArgs[0] == "--host");
    assert(cmdArgs[1] == "mydomain.com");
    assert(cmdArgs[5] == r"D:\my path");
    
    cmdArgs.length = 0;
    cmd = `--local "D:\my path" --host mydomain.com`;
    parseCommands(cmd, cmdArgs);
    assert(cmdArgs.length == 4);
    assert(cmdArgs[1] == r"D:\my path");
    
    cmdArgs.length = 0;
    cmd = `"--local" "D:\my path"`;
    parseCommands(cmd, cmdArgs);
    assert(cmdArgs.length == 2);
    assert(cmdArgs[0] == "--local");
    assert(cmdArgs[1] == r"D:\my path");
    ...
    cmd = "--local";
    parseCommands(cmd, cmdArgs);
    assert(cmdArgs.length == 1);
    assert(cmdArgs[0] == "--local");
    
    //Stdout.formatln("parseCommands unittest passed");
}

/*
void readCmd(char[] cmd, ref char[][] cmdArgs)
{
    cmd = TextUtil.trim(cmd);
    if(!cmd.length || cmd == "\"\"") return;
    if(cmd[0] == '"')
    {
        if(cmd.length > 2 && cmd[0..2] == "\"\"")
            return readCmd(cmd[2..$], cmdArgs);
        foreach(index, c; cmd)
        {
            if(index > 0 && c == '"')
            {
                ...
                cmd = cmd[index+1..$];
                if(cmd.length > 1)
                    return readCmd(cmd, cmdArgs);
                else
                    return;
            }
        }
        //return readCmd(cmd[1..$], cmdArgs);
    }
