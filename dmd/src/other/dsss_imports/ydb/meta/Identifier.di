...
{
	const char[] prettyNameOf = toString (demangleIdentifier !(A), show_type, 1, 1);
}
...
{
	const char[] fullyQualifiedNameOf = toString (demangleIdentifier !(A), false, 0, byte.max);
}
...
{
	const char[] accessNameOf = getAccessName (demangleIdentifier !(A));
}
...
/// Worker template for isFunction
