...
/// Worker template for demangleIdentifier
...
public char[] toString (Demangle demangle, bool show_type, byte function_depth = 1, byte template_depth = 1, bool main_call = true)
{
	...
	case Demangle.IdentifierTyped:
		...
		if (function_depth > 0 && (demangle.sub[0].type == Demangle.TypeFunction || demangle.sub[0].type == Demangle.TypeDelegate) && (demangle.idata & 32) == 0)
		{
			...
			{
				result ~= (i > 0 ? ", " : "") ~ toString (arg, false, function_depth-1, template_depth-1, false);
			}
			result ~= ")";
		}
		return result;
...
private char[] getPrefix (Demangle demangle, byte function_depth, byte template_depth)
{
	switch (demangle.type)
	{
	case Demangle.IdentifierSimple:
		return demangle.cdata;
	case Demangle.IdentifierTyped:
		char[] result = getPrefix (demangle.sub[0], function_depth, template_depth);
...
public char[] debugString (Demangle demangle, char[] indent = "", bool already_indended = false)
{
	char[] first_indent = already_indended ? "" : indent;
	...
	case Demangle.IdentifierTemplate:
		char[] result = first_indent ~ demangle.cdata ~ " (Template):\n";
		foreach (i, arg; demangle.sub)
			result ~= debugString (arg, indent ~ "  ", false) ~ (i+1 < demangle.sub.length ? "\n" : "");
		return result;
	case Demangle.IdentifierQualifiedName:
		char[] result = first_indent ~ "{\n";
		foreach (i, ident; demangle.sub)
	...
	case Demangle.TypeTypedef:
		return "Typedef:\n" ~ debugString (demangle.sub[0], indent ~ "  ", false);
	case Demangle.TypeFunction:
		char[] result = Demangle.printExtern (demangle.idata);
...
private char[] getBaseString (Demangle demangle)
{
	switch (demangle.type)
	{
...
public char[] getAccessName (Demangle demangle)
{
	switch (demangle.type)
	{
	case Demangle.Invalid:
	...
	case Demangle.IdentifierTyped:
		return demangle.cdata;
	case Demangle.IdentifierQualifiedName:
		char[] result = getAccessName (demangle.sub[$-1]);
...
public:
	...
	static Demangle opCall (int type, Demangle[] sub = [], long idata = 0, char[] cdata = [], real re = 0.0, real im = 0.0)
	{
	...
	static Demangle opCall (char[] error_message)
	{
	...
	private static char[] printExtern (long flag)
	{
		switch (flag & 7)
		{
			case 0: return "";
...
template demangleType (char[] str, char[] indent = "")
{
...
private template demangleType_ (char[] str, char[] indent = "")
{
	debug (Demangle)
...
template demangleFunction (char[] str, char[] indent = "")
{
...
private template demangleFunction_ (int type, char[] str, char[] indent = "", bool is_anonymous)
{
	debug (Demangle)
...
template demangleFunctionArguments (char[] str, char[] indent = "")
{
...
private template demangleFunctionArguments_ (char[] str, char[] indent = "")
{
	debug (Demangle)
...
template demangleQualifiedName (char[] str, char[] indent = "")
{
...
private template demangleQualifiedName_ (char[] str, char[] indent = "")
{
	debug (Demangle)
		pragma (msg, indent ~ "demangleQualifiedName (\"" ~ str ~ "\")");
	
	static if (str.length > 1 && str[0..2] == "_D") const char[] str2 = str[2..$];
	else const char[] str2 = str;
...
template demangleSymbol (char[] str, char[] indent = "")
{
...
private template demangleSymbol_ (char[] str, char[] indent = "")
{
	debug (Demangle)
...
template demangleLName (char[] str, char[] indent = "")
{
	debug (Demangle)
		pragma (msg, indent ~ "demangleLName (\"" ~ str ~ "\")");

	const long temp_size = decToInt (str);
	const char[] temp_remaining = consumeDecInt (str);

	const char[] name = temp_remaining [0..temp_size];
	const remaining = temp_remaining [temp_size..$];
}

/// Demangle the mangle of template parameters.

template demangleTemplateArguments (char[] str, char[] indent = "")
{
...
private template demangleTemplateArguments_ (char[] str, char[] indent = "")
{
	debug (Demangle)
...
template demangleValue (char[] str, char[] indent = "")
{
...
private template demangleValue_ (char[] str, char[] indent = "")
{
	debug (Demangle)
	...
	{
		const long temp_number = decToInt (str[1..$], "");
		const char[] temp_remaining = consumeDecInt (str[1..$], "");
...
private template demangleHexFloat_ (char[] str, char[] indent = "")
{
	debug (Demangle)
...
package real parseHexFloat (char[] str)
{
	if (str[0] == 'N')
...
package char[] consumeHexFloat (char[] str)
{
