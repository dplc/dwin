...
version(DigitalMars) version(Posix) {
    version=Unix;
}
...
bool isNumeric(TypeInfo[] _arguments, void* _argptr)
{
    ...
    if (_arguments[0] == typeid(char[]))
        return isNumeric(va_arg2!(char[])(_argptr));
    else if (_arguments[0] == typeid(wchar[]))
        return isNumeric(std.utf.toUTF8(va_arg2!(wchar[])(_argptr)));
    else if (_arguments[0] == typeid(dchar[]))
        return isNumeric(std.utf.toUTF8(va_arg2!(dchar[])(_argptr)));
    ...
    {
        s.length = 1;
        s[0] = va_arg2!(char)(_argptr);
        return isNumeric(s);
    }
    else if (_arguments[0] == typeid(wchar))
    {
        ws.length = 1;
        ws[0] = va_arg2!(wchar)(_argptr);
        return isNumeric(std.utf.toUTF8(ws));
    }
    else if (_arguments[0] == typeid(dchar))
    { 
        ds.length =  1;
        ds[0] = va_arg2!(dchar)(_argptr);
