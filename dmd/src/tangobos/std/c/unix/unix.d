...
public import tango.stdc.errno: EPERM, ENOENT, ESRCH, EINTR, EIO, ENXIO, E2BIG, ENOEXEC,
    EBADF, ECHILD, ENOMEM, EACCES, EFAULT, EBUSY, EEXIST, EXDEV, ENODEV, ENOTDIR, EISDIR,
    EINVAL, ENFILE, ENOTTY, EFBIG, ESPIPE, EROFS, EMLINK, EPIPE, EDOM, ERANGE, EDEADLK,
    ENAMETOOLONG, ENOLCK, ENOSYS, ELOOP, ENOMSG, EIDRM, ENOLINK, EPROTO, EMULTIHOP,
    EBADMSG, EOVERFLOW, EILSEQ, ENOTSOCK, EDESTADDRREQ, EMSGSIZE, EPROTOTYPE, ENOPROTOOPT,
    EPROTONOSUPPORT, EADDRINUSE, EADDRNOTAVAIL, ENETDOWN, ENETUNREACH, ENETRESET, ECONNABORTED,
    ECONNRESET, ENOBUFS, EISCONN, ENOTCONN, ETIMEDOUT, ECONNREFUSED, EHOSTUNREACH, EALREADY,
    EINPROGRESS, ESTALE, ECANCELED;
// these are the errors in common to all unix systems, unfortunately it is not possible to
// include the whole errno due to conflicts with getErrno and setErrno
...
} else version(DigitalMars) version(Posix){
public import std.c.linux.linux;
public import std.c.linux.socket;
version (linux) public import std.c.dirent: readdirD;
}
