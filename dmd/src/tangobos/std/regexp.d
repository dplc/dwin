...
{
    debug(regexp) {
        version(DigitalMars) printf("regexp.exec(string = '%.*s')\n", string);
        else
        printf("regexp.exec(string = '%.*s')\n",
...
public int test(char[] string, int startindex)
{
    ...
    input = string;
    debug (regexp) {
        version (DigitalMars) printf("RegExp.test(input[] = '%.*s', startindex = %d)\n", input, startindex);
        else printf("RegExp.test(input[] = '%.*s', startindex = %d)\n",
...
void printProgram(ubyte[] prog)
{
  //debug(regexp)
  {
	...
	switch (prog[pc])
	{
		...
		version (DigitalMars) printf("\tREstring x%x, '%.*s'\n", len,s);
		else printf("\tREstring x%x, '%.*s'\n", len,
...
{   int srcsave;
	...
	switch (program[pc])
	{
		...
		{
		    char[] s = (&program[pc + 1 + uint.sizeof])[0 .. len];
		    version (DigitalMars) printf("\tREistring x%x, '%.*s'\n", len,s);
		    else printf("\tREistring x%x, '%.*s'\n", len,
			    cast(int) s.length, s.ptr);
		}
		if (src + len > input.length)
		    goto Lnomatch;
