...
uint adler32(Culong_t adler, void[] buf)
{
    return etc.c.zlib.adler32( adler, cast(ubyte *)buf.ptr, cast(uint) buf.length);
}
...
uint crc32(Culong_t crc, void[] buf)
{
    return etc.c.zlib.crc32(crc, cast(ubyte *)buf.ptr, cast(uint) buf.length);
}
...
body
{
    ...
    destlen = etc.c.zlib.compressBound(cast(Culong_t)srcbuf.length);
    destbuf = new ubyte[destlen];
    err = etc.c.zlib.compress2(cast(ubyte *)destbuf.ptr, &destlen, 
            cast(ubyte *)srcbuf.ptr, cast(Culong_t)srcbuf.length, level);
    if (err)
    {	delete destbuf;
